. 다음 두 명령어는 어떤 기능을 수행하는 명령어인지 작성하고,
두 기능의 차이점이 있다면 설명하시오.
DELETE FROM 테이블명;
TRUNCATE TABLE 테이블명;

delete from    : 데이터 삭제, DML, 조건에 따라서 원하는 데이터만 삭제(where 조건), 수행 후 commit/rollback 기회
truncate table : 데이터 삭제, DDL, 테이블 전체 삭제(초기화), 삭제쿼리 실행시 바로 적용(auto commit)
